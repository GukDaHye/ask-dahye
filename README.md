...
| **GitHub MCP는 원격 HTTP 서버** | 코드 질문은 문서 대신 실제 리포 파일을 근거로 씁니다. 로컬 Docker나 바이너리를 띄우는 방식 대신 원격 streamable HTTP를 골라, 상시 프로세스 배포 환경에 추가 인프라 없이 올라갑니다. 서버 쪽 `X-MCP-Readonly` 헤더로 쓰기 도구를 한 겹 더 차단합니다. |
...
- **MCP 대상 리포가 하나입니다.** `GITHUB_REPO` 하나만 보므로, 지식베이스가 다루는 6개 프로젝트 중 한 리포의 코드만 답할 수 있습니다. "티켓팅 코드"를 물어도 다른 리포를 가리키고 있으면 2차 게이트가 "근거 없음"으로 막습니다(틀린 답을 하지는 않지만 답도 못 합니다). 프로젝트별 리포 매핑이 다음 개선점입니다.
...
GitHub MCP 연동은 선택입니다. 아래 두 값을 채우면 코드 질문이 실제 리포 파일을 근거로 답하고, 비워두면 코드 질문도 기존 RAG 경로로 처리됩니다. 토큰은 fine-grained PAT에 `Contents: Read-only`만 주면 됩니다.
