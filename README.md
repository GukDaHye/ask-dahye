...
목적이 두 가지입니다. 하나는 "티켓팅 인프라에서 왜 그 기술을 선택했나요?" 같은 구체적인 질문에 문서를 근거로 바로 답하게 하는 것입니다. 다른 하나는 LLM 서빙 백엔드에서 실제로 부딪히는 문제 — 스트리밍 중단, 재시도 경계, 429 처리 — 를 직접 겪고 검증해보는 것입니다.

그래서 프레임워크로 감싸지 않고 각 단계를 직접 구현했습니다. 이 챗봇 자체가 백엔드 작업물이기도 합니다.
...
```mermaid
flowchart TD
    A["질문 입력<br/>(세션 ID 동반)"] --> B["검색 쿼리 조립<br/>직전 질문 + 현재 질문"]
    ...
    E -- "1~5개" --> G["프롬프트 조립<br/>청크 전체 + 최근 3턴 + 질문"]
    G --> H["동시 실행 상한<br/>Semaphore(1)"]
    H --> I["LLM 스트리밍 요청<br/>재시도: 5xx · 429 · 타임아웃"]
    I -- "전부 실패" --> J["fallback<br/>검색된 청크 링크 안내"]
    I -- "성공" --> K["SSE 릴레이<br/>토큰 그대로 전달"]
    K --> L{"2차 게이트<br/>LLM이 근거 있다고 판단?"}
    L -- "아니오" --> M["No-Info 톤<br/>출처 칩 숨김"]
    L -- "예" --> N["답변 + 출처 칩<br/>상위 3개"]
```
...
| `main.py` | FastAPI 서버. 검색 → 프롬프트 조립 → LLM 스트리밍 릴레이(SSE), 재시도·큐잉·fallback·rate limit |
