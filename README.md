...
| MCP 근거 수집 (첫 질문, 목록 캐시 없음) | 3.80초 (4회 중앙값, 3.7~4.3초) |
| MCP 근거 수집 (목록 캐시 적용) | 0.67초 (8회 중앙값, 0.66~1.09초) |
...
TTFT·검색·fallback 도달 시간은 네트워크 상황에 따라 실행마다 편차가 있어 관측 범위로 적었습니다. fallback은 6회, MCP 근거 수집은 4회 실행의 중앙값이며 실행별 값은 [`docs/bench_retry_runs.md`](docs/bench_retry_runs.md)와 [`docs/bench_mcp_runs.md`](docs/bench_mcp_runs.md)에 있습니다. 재현 스크립트는 `scripts/` 아래에 있습니다.
...
| **파일 목록 5분 캐싱** | 코드 질문 3.8초 → 0.7초 | 커밋 직후 즉시 반영이 필요할 때. `LISTING_TTL_SECONDS` 한 줄입니다. |
